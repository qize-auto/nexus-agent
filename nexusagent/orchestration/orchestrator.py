...
class Orchestrator:
    ...
    def __init__(
        ...
        profile_adapter_registry: Any = None,  # ProfileAdapterRegistry (v4.0+ 统一注册)
        ...
        self._profile_adapter_registry = profile_adapter_registry

        # 优先从 registry 获取 adapter（v4.0+ 统一注册机制），否则回退到旧参数
        if profile_adapter_registry is not None:
            self._swarm_profile_adapter = swarm_profile_adapter or profile_adapter_registry.get("swarm")
            self._memory_profile_adapter = memory_profile_adapter or profile_adapter_registry.get("memory")
            self._react_profile_adapter = react_profile_adapter or profile_adapter_registry.get("react")
            self._guardrails_profile_adapter = guardrails_profile_adapter or profile_adapter_registry.get("guardrails")
            self._tools_profile_adapter = tools_profile_adapter or profile_adapter_registry.get("tools")
        else:
    ...
    async def process(
        ...
        # ── 1. 输入审查 ──
        if hasattr(self._guardrails, 'areview'):
            input_review = await self._guardrails.areview(message)
        else:
