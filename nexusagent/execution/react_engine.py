...
class ReActEngine:
    ...
    def __init__(
        ...
        self._default_budget = self._budget  # 保存默认预算引用，用于覆盖后恢复
    ...
    async def run(
        ...
        budget_override: Optional[ReActBudget] = None,
        ...
        if budget_override is not None:
            ...
            # 安全底线: max_iterations >= 10, max_time_seconds >= 60
            budget_override.max_iterations = max(10, budget_override.max_iterations)
            budget_override.max_time_seconds = max(60.0, budget_override.max_time_seconds)
            self._budget = budget_override
            logger.debug("ReActBudget overridden: iter=%d, time=%.0fs", budget_override.max_iterations, budget_override.max_time_seconds)
        ...
        tool_descriptions = tools_override if tools_override is not None else self._tools.describe_tools()
