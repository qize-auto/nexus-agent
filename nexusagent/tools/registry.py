...
class ToolRegistry:
    ...
    def discover_plugins(self) -> int:
        ...
        Registry 主导扫描和注册，PluginManager 提供元数据管理和增强信息。
        """
        # PluginManager 元数据层 — 发现并缓存插件信息
            ...
            for finder, name, ispkg in pkgutil.iter_modules():
                if name.startswith("nexus_plugin_"):
                    try:
                        ...
                        # 同步注册到 PluginManager（若尚未注册）
                        if name not in [p.name for p in pm.list_plugins()]:
                            pm.load_plugin(name)
                    except Exception as e:
        ...
        # 输出 PluginManager 提供的元数据摘要
        if pm.list_plugins():
            logger.info("PluginManager 元数据: %d 个插件已发现", len(pm.list_plugins()))
        return count
